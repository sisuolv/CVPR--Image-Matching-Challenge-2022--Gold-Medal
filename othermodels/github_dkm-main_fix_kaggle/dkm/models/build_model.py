...
def dkm_base(pretrained=True, version="v11", request_im_size=(384, 512), use_cuda=True):
    gp_dim = 256
    dfn_dim = 384
    feat_dim = 256
    ...
    if use_cuda:
        ...
        matcher = RegressionMatcher(encoder, decoder, h=h, w=w).cpu()
    if pretrained:
        if use_cuda:
            weights = torch.hub.load_state_dict_from_url(dkm_pretrained_urls["base"][version])
        else:
            weights = torch.hub.load_state_dict_from_url(dkm_pretrained_urls["base"][version], map_location=torch.device('cpu'))
