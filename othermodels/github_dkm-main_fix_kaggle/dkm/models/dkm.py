        ...
        with torch.no_grad():
            if not batched:
                ...
                if use_cuda:
                    ...
                    batch = {"query": query[None], "support": support[None]}
            else:
                b, c, h, w = im1.shape
                b, c, h2, w2 = im2.shape
            ...
            if do_pred_in_og_res:  # Will assume that there is no batching going on.
                ...
                query_to_support, dense_certainty = self.matcher.upsample_preds(
                    query_to_support,
                    dense_certainty,
                    og_query.cuda()[None] if use_cuda else og_query[None],
                    og_support.cuda()[None] if use_cuda else og_support[None],
                )
                hs, ws = h, w
            # Create im1 meshgrid
            query_coords = torch.meshgrid(
                (
                    torch.linspace(-1 + 1 / hs, 1 - 1 / hs, hs, device="cuda:0" if use_cuda else "cpu"),
                    torch.linspace(-1 + 1 / ws, 1 - 1 / ws, ws, device="cuda:0" if use_cuda else "cpu"),
                )
            )
